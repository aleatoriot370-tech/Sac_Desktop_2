...
from core.database import listar_investigacoes_abertas
...
class InvestigacoesAbertas(BasePopupModule):
    """Módulo 1.2.1.2 - Investigações Abertas: OS com Tipo F ou Q, Status 'Em Investigação'."""

    def __init__(self, usuario_logado: dict, parent=None):
        super().__init__(usuario_logado, parent)
        self.setWindowTitle("Aprovações - Investigações Abertas")
        self.resize(760, 520)
    ...
    def _montar_ui(self):
        ...
        titulo = QLabel("Investigações Abertas")
        titulo.setObjectName("tituloTela")
        layout.addWidget(titulo)

        dica = QLabel("Dê duplo clique em uma linha para abrir a investigação.")
        ...
        self.tabela.cellDoubleClicked.connect(self._abrir_investigacao)
    ...
    def _carregar(self):
        try:
            self._resultados = listar_investigacoes_abertas()
    ...
    def _abrir_investigacao(self, linha: int, _coluna: int):
        registro = self._resultados[linha]
        os_id = registro["os_id"]

        if registro["tipo"] == "F":
            dialogo = InvestigacaoPF(self.usuario_logado, os_id, parent=self)
        else:
            dialogo = InvestigacaoPJ(self.usuario_logado, os_id, parent=self)
